...
- **Все тесты плагина:** `bin/rspec plugins/discourse-anonymous-plugin/`
- **Один файл:** `bin/rspec plugins/discourse-anonymous-plugin/spec/path/to/spec.rb`
- **Один пример:** `bin/rspec plugins/discourse-anonymous-plugin/spec/path/to/spec.rb:42`
...
- **Именование:** хелпер — `AnonymousPostHelper` (модуль верхнего уровня, `::` prefix), остальные — `AnonymousPost::*`
- **Кеш anonymous_user:** `AnonymousPostHelper.anonymous_user` кешируется в-памяти; `reset_cache!` нужен при изменении `anonymous_post_user` в тестах
- **Ограничение ответов:** анонимно ответить может только автор топика в своей анонимной теме (не другие пользователи)
